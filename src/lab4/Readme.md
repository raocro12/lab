...
(задача 3.1.1 Обобщенная коробка.  
...
создания объекта)  
### Алгоритм решения
1. Создадим обобщённый класс `public class Box<T>` так как коробка должна иметь воможность хранить данные любого типа.
...
2. Метод, который принимает коробку и кладет в неё трёхмерную точку `public static void putPoint(Box<? super Point3D> box)`. Метод может принимать только коробку типа `Point3D` и всех её родительских классов. По условию задачи метод всегда кладёт в коробку трёхмерную точку с произвольными знаениями, то есть можем не передавать точку, а сразу в методе положить в коробку 3 мерную точку с рандомными значениями.  
3. В главном методе создаём экземпляр коробки `Box<Point3D> box = new Box<>();` и кладём точку `PointInBox.putPoint(box);`.
### Тесты
