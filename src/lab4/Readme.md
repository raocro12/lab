...
Метод, который принимает коробку и кладет в неё трёхмерную точку `public static void putPoint(Box<? super Point3D> box)`. Метод может принимать только коробку типа `Point3D` и всех её родительских классов. По условию задачи метод всегда кладёт в коробку трёхмерную точку с произвольными знаениями, то есть можем не передавать точку, а сразу в методе положить в коробку 3 мерную точку с рандомными значениями.  
